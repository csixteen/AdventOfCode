...
impl Solution {
    fn total_square_feet(lines: &[&str]) -> i32 {
        lines
            .iter()
            .fold(0, |acc, d| acc + Dimension::from_str(d).square_feet())
    }

    fn total_ribbon(lines: &[&str]) -> i32 {
        lines
            .iter()
            .fold(0, |acc, d| acc + Dimension::from_str(d).ribbon())
    }
}
